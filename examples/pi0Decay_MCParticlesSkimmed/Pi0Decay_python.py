...
df = df.ReadEvent("event", ["MCParticles", "MCParticlesSkimmed"], throwExceptionOnRefCollIDNotFound=True)
#df = df.ReadEvent("event")

df = df.Define("ans", "analyzeEvent(event)")
df = df.Define("notfIDS", "event.getNotFoundIDs()")

dfa = df.AsNumpy_(["notfIDS"]).GetValue()

print("not found IDs", dfa.notfIDS[0])
